...
if __name__ == "__main__":
    ...
    field = str(raw_input("select query field > "))
    value = str(raw_input("select value > "))
    del_field = str(raw_input("select delete field > "))
    ...
    while (result_count > 0):
        ...
        if value is not '' and value is not None:
            post_data = json.dumps({"size": 1000, "query": {"match": {field: value}}})
        else:
